...
export default function ApplyDialog({ jobId, isOpen, setIsOpen }) {
    const [coverLetter, setCoverLetter] = useState("");
    const [file, setFile] = useState(null);
    const [loading, setLoading] = useState(false);
    const [previewUrl, setPreviewUrl] = useState(null);

    // Xóa URL object khi component bị unmount để tránh rò rỉ bộ nhớ
    React.useEffect(() => {
        return () => {
            if (previewUrl) {
    ...
    const handleUpload = (e) => {
        ...
        if (uploadedFile) {
            ...
            // Nếu có preview cũ, thu hồi nó trước khi tạo cái mới
            if (previewUrl) {
                URL.revokeObjectURL(previewUrl);
            }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            toast.error("Gửi CV thất bại, vui lòng thử lại");
        } finally {
            setLoading(false);
        }
    };
                ...
                <AnimatePresence>
                    {isOpen && (
                        <>
                            ...
                            {/* Bọc trong một div flex để căn giữa */}
                                    ...
                                    >
                                        <div className="flex justify-between items-center mb-4">
                                            ...
                                            <Dialog.Close asChild>
                                                <button className="p-2 rounded-full text-gray-500 hover:bg-gray-100 dark:hover:bg-neutral-700 transition" aria-label="Đóng">
                                                    <X className="w-5 h-5" />
                                                </button>
                                            </Dialog.Close>
                                        </div>
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            {/* Upload CV */}
                                            <div>
                                                <label htmlFor="resumeFile" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Tải lên CV (PDF)</label>
                                                <input
                                                    id="resumeFile"
                                                    type="file"
                                                    accept=".pdf"
                                                ...
                                                {file && (
                                                    <div className="mt-2 flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                                                        ...
                                                        <a href={previewUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline flex items-center gap-1 ml-auto">
                                            ...
                                            <div className="flex justify-end gap-3 pt-2">
                                                <Dialog.Close asChild>
                                                    <button type="button" className="px-4 py-2 rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-50 dark:border-neutral-600 dark:text-gray-300 dark:hover:bg-neutral-700">
                                                        Hủy
                                                    </button>
                                                </Dialog.Close>
                                                <button
                                                    type="submit"
                                                    disabled={loading}
                                                    className="px-4 py-2 rounded-lg bg-orange-500 text-white hover:bg-orange-600 flex items-center gap-2 disabled:bg-orange-300"
                                                >
                                                    {loading ? (
