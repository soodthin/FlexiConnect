...
useEffect(() => {
    async function loadProfile() {
      try {
        ...
        setProfile(res.data); // res.data có isVerified
      } catch (err) {
    ...
    <div className="w-full bg-white dark:bg-neutral-900 rounded-xl shadow p-6 text-[16px] leading-relaxed text-gray-800 dark:text-gray-100">
      ...
      <Dialog.Root open={open} onOpenChange={setOpen}>
        <Dialog.Trigger asChild>
          <button
            className={`
              flex items-center gap-2 px-5 py-3 rounded-xl font-semibold shadow mb-4
              ${profile?.isVerified
                ? "bg-neutral-900 dark:bg-beige dark:text-black text-white hover:bg-neutral-800 dark:hover:bg-[#f5f5dc]"
