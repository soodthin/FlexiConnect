...
public ResponseEntity<?> update(Authentication auth, @PathVariable Integer id, @RequestBody EducationHistory edu) {
    String email = auth.getName();
    User user = userRepository.findByEmail(email)
            .orElseThrow(() -> new RuntimeException("User not found"));
    return ResponseEntity.ok(educationHistoryService.update(user, id, edu));
}
